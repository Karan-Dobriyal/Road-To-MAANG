...
public class TowerOfHanoi {
    public void  towerOfHanoi() {
     ...
     public static void hanoi(ArrayList<ArrayList<Integer>> list,int A,int source, int destination,int helper){
        if(A==1){
        ...
        hanoi(list,A-1,source,helper,destination);
        ArrayList<Integer> l = new ArrayList<Integer>();
        l.add(A);
        l.add(source);
        l.add(destination);
        list.add(l);
        hanoi(list,A-1,helper,destination,source);
    }
}
// Time Complexity : O(2𝑛) and Space Complexity : O(𝑛)
